...
from app.services.upload_service import process_uploaded_image, process_uploaded_video
...
async def detect_image_upload(file: UploadFile = File(...)):
    try :
        result = await process_uploaded_image(file, "image_upload")
...
async def detect_image_snapshot(file: UploadFile = File(...)):
    try :
        result = await process_uploaded_image(file, "image_snapshot")
        return {"result": result}
    except Exception as e:
        return {"error": str(e)}
...
async def detect_video_upload(file: UploadFile = File(...)):
    try :
        video_url = await process_uploaded_video(file)
