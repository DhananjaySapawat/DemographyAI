...
export NEXT_PUBLIC_BASE_URL="http://$IP:$MONITOR_PORT"
export NEXT_PUBLIC_BACKEND_URL="http://$IP:$BACKEND_PORT"

echo "[monitor] url=$NEXT_PUBLIC_BASE_URL  backend=$NEXT_PUBLIC_BACKEND_URL"
...
case "$LAUNCH_MODE" in
  ...
  prod)
    ...
    ;;
  *)
    echo "[monitor] ERROR: unknown LAUNCH_MODE '$LAUNCH_MODE'" >&2
    exit 1
    ;;
esac